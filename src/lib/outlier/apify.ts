...
export type PullResult = { posts: RawPost[]; avatarUrl: string | null };
...
export async function pullTikTok(handle: string, token: string, limit = 30): Promise<PullResult> {
  ...
  const filtered = items.filter((item) => item && item.id && !item.error);

  const posts: RawPost[] = filtered.map((item) => ({
    externalId: String(item.id),
    caption: item.text ?? "",
    url: item.webVideoUrl ?? "",
    // The scraper doesn't return a direct video file by default — getting
    // one costs an extra, targeted Apify call (see fetchTikTokVideoUrl),
    // made lazily only for posts someone actually asks to analyze.
    videoUrl: null,
  ...
  return { posts, avatarUrl: filtered[0]?.authorMeta?.avatar ?? null };
}
...
export async function pullInstagram(handle: string, token: string, limit = 30): Promise<PullResult> {
  const items = await runActor(
  ...
  const filtered = items.filter((item) => item && item.id && (item.videoPlayCount != null || item.videoViewCount != null));
  ...
  // Not returned by this actor's "posts" mode without extra options — no
  // avatar captured for Instagram yet.
  return { posts, avatarUrl: null };
}
...
export async function pullYouTube(handle: string, token: string, limit = 30): Promise<PullResult> {
...
// Resolves a downloadable video file for exactly one TikTok post, only
// called when someone explicitly asks to analyze that post — downloading
// every pulled post's video via Apify's shouldDownloadVideos add-on would
// be a real, avoidable cost multiplied across an entire pull.
export async function fetchTikTokVideoUrl(postUrl: string, token: string): Promise<string | null> {
  const items = await runActor(
    "clockworks~tiktok-scraper",
    {
      postURLs: [postUrl],
